...
const EditAsset = () => {
  ...
  useEffect(() => {
    const fetchAsset = async () => {
      try {
        ...
        // Remove nested fields or fields that shouldn't be sent back in PUT
        const { assignments, qr_code, created_at, ...data } = res.data;
        setFormData(data);
        setLoading(false);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => navigate(`/asset/${id}`), 2000);
    } catch (err) {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 gap-4">
      ...
      <p className="text-slate-500 font-black uppercase tracking-[0.3em] text-xs">Accessing Secure Records</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <header className="mb-12">
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight mb-2">Modify <span className="text-violet-500">Asset Parameters</span></h1>
          <p className="text-slate-500 font-medium">Update the technical specifications for hardware identity <span className="text-white">#{id}</span>.</p>
        </header>
        ...
        <div className="glass p-8 md:p-12 border-white/5">
          {success ? (
            ...
            >
              ...
              <h2 className="text-3xl font-black text-white mb-2 uppercase">Record Updated</h2>
              <p className="text-slate-500">Synchronizing database and updating identity protocols...</p>
            </motion.div>
          ) : (
              ...
              <div className="pt-8 border-t border-white/5">
                ...
                >
                  {saving ? <Loader2 className="animate-spin" /> : (
                    <>
                      Update Secure Record <Save size={18} />
                    </>
                  )}
                </button>
              </div>
